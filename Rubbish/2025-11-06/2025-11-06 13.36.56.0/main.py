...
# Configure CORS for both localhost and network IP
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173", 
...
async def create_product(
    ...
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_admin_or_super)
):
    """Create new product"""
    # Check if code already exists
    db_product = db.query(Product).filter(Product.codigo == product.codigo).first()
    if db_product:
...
async def create_client(
    ...
    db_client = db.query(Client).filter(Client.documento == client.documento).first()
    if db_client:
...
async def get_dashboard_metrics(
    ...
    # Total clients
    total_clientes = db.query(Client).filter(Client.is_active == True).count()
