...
export default function Layout({ children, user, onLogout, currentPage, onNavigate }: LayoutProps) {
  ...
  const filteredMenuItems = menuItems.filter(item => {
    if (item.superUserOnly && !isSuperUser(user)) {
      console.log(`Filtered out ${item.id} - superUserOnly`);
      return false;
    }
    const hasAccess = hasPermission(user, item.permission);
    console.log(`Menu ${item.id}: permission=${item.permission}, hasAccess=${hasAccess}`);
    return hasAccess;
  });
